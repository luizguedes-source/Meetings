...
st.set_page_config(page_title="SPIN Analytics & Conversion", layout="wide", initial_sidebar_state="expanded")
...
SPIN_KEYWORDS = {
    "S": ["contexto", "situação", "atualmente", "processo", "cenário", "estrutura", "equipe", "hoje", "operacional", "rotina"],
    "P": ["problema", "dificuldade", "desafio", "dor", "gargalo", "ruim", "falta", "prejuízo", "erro", "morosidade", "lento", "burocracia"],
    "I": ["consequência", "impacto", "risco", "custo", "perda", "afeta", "atraso", "piora", "multa", "processo trabalhista", "juros"],
    "N": ["solução", "ideal", "investimento", "resultado", "melhoria", "meta", "objetivo", "crescimento", "expectativa", "resolver", "ganho"]
}

NEXT_STEPS_KEYWORDS = ["proposta", "contrato", "fechamento", "assinar", "agendar", "retorno", "pagamento", "onboarding", "avançar", "aprovado"]
DEMO_JSON_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "meetings_sample.json")
...
@st.cache_data(show_spinner="Minerando diretórios 001 ao 018 e analisando as reuniões (pode levar alguns minutos, tenha paciência!)...")
def load_meetings_data(base_path, limit=250):
    ...
    for folder in folders:
        ...
        meeting_info = {
            ...
            "title": "Desconhecido",
            "S_score": 0, "P_score": 0, "I_score": 0, "N_score": 0,
            "SPIN_total": 0,
            "avancou_funil": False,
        ...
        if p_count >= 5: meeting_info["P_score"] = 3
        elif p_count >= 2: meeting_info["P_score"] = 2
        elif p_count == 1: meeting_info["P_score"] = 1
        
        i_count = sum(corpus.count(w) for w in SPIN_KEYWORDS["I"])
        if i_count >= 3: meeting_info["I_score"] = 3
        elif i_count >= 1: meeting_info["I_score"] = 2
        
        n_count = sum(corpus.count(w) for w in SPIN_KEYWORDS["N"])
        meeting_info["N_score"] = 2 if n_count >= 2 else (1 if n_count == 1 else 0)
        
        meeting_info["SPIN_total"] = meeting_info["S_score"] + meeting_info["P_score"] + meeting_info["I_score"] + meeting_info["N_score"]
        
        adv_count = sum(corpus.count(w) for w in NEXT_STEPS_KEYWORDS)
        if adv_count >= 1 or any(x in meeting_info["title"].lower() for x in ["fechamento", "onboarding", "faturamento"]):
...
# Layout Principal
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Visão Geral", "🛠️ Análise Isolada", "💡 Padrões", "☁️ Nuvem", "📋 Frequência", "🎯 Recomendações", "📈 Forecast"])
...
with tab1:
    ...
    with res_col1:
        ...
        df["Score_Bucket"] = pd.cut(df["SPIN_total"], 
                                    bins=[-1, 3, 6, 8, 10], 
                                    labels=["Baixo (0-3)", "Médio (4-6)", "Alto (7-8)", "Ótimo (9-10)"])
        conv_by_bucket = df.groupby("Score_Bucket", observed=True)["avancou_funil"].mean().reset_index()
        conv_by_bucket["avancou_funil"] = conv_by_bucket["avancou_funil"].fillna(0) * 100
        fig_bar = px.bar(conv_by_bucket, x="Score_Bucket", y="avancou_funil", 
                     text_auto='.1f', labels={"avancou_funil": "Taxa de Avanço (%)", "Score_Bucket": "Faixa de SPIN Score"},
                     color="Score_Bucket", color_discrete_sequence=px.colors.sequential.Blues, height=350)
        fig_bar.update_layout(showlegend=False, xaxis_title="SPIN Score", yaxis_title="% Avançou")
        st.plotly_chart(fig_bar, use_container_width=True)
        
    with res_col2:
        st.markdown("**Radar Comparativo: Reuniões que Avançam vs Travam**")
        mean_scores = df.groupby("avancou_funil")[["S_score", "P_score", "I_score", "N_score"]].mean().reset_index()
        if len(mean_scores) == 2:
            fig_radar = go.Figure()
            categories = ['Situação', 'Problema', 'Implicação', 'Necessidade']
            sucesso = mean_scores[mean_scores["avancou_funil"] == True].iloc[0][1:].values.tolist()
            fracasso = mean_scores[mean_scores["avancou_funil"] == False].iloc[0][1:].values.tolist()
            fig_radar.add_trace(go.Scatterpolar(r=sucesso, theta=categories, fill='toself', name='Avançou (Sucesso)', line_color='#1f77b4', opacity=0.8))
            fig_radar.add_trace(go.Scatterpolar(r=fracasso, theta=categories, fill='toself', name='Travou (Estagnou)', line_color='#d62728', opacity=0.8))
            fig_radar.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 3])), showlegend=True, height=350, margin=dict(t=0, b=0, l=0, r=0))
...
with tab2:
    ...
    df_view = df_view[["title", "SPIN_total", "S_score", "P_score", "I_score", "N_score", "avancou_funil", "arquivos"]]
...
with tab3:
    ...
    st.markdown("Veja matematicamente como a presença das fases do método SPIN mudam o comportamento no funil da Villela do Lote analisado.")
...
with tab4:
    ...
    st.markdown("Termos de negócio mais comentados. Artigos, pronomes e gírias de conversa foram estritamente removidos da Nuvem.")
    
    if not HAS_WORDCLOUD:
        st.error("⚠️ Você precisa instalar a biblioteca `wordcloud` e `matplotlib`. Execute isso no seu terminal: `pip install wordcloud matplotlib`")
    else:
        stopwords_pt = set([
            "a", "o", "e", "que", "do", "da", "em", "um", "para", "com", "não", "uma", "os", "no", "se", "na",
            "por", "mais", "as", "dos", "como", "mas", "ao", "das", "à", "seu", "sua", "ou", "quando", "já",
            "muito", "nos", "eu", "também", "só", "pelo", "pela", "até", "isso", "ela", "entre", "depois",
            "sem", "mesmo", "aos", "seus", "quem", "nas", "me", "esse", "eles", "você", "essa", "num", "nem",
            "suas", "meu", "às", "minha", "numa", "pelos", "elas", "qual", "nós", "lhe", "deles", "essas", "esses",
            ...
            "tem", "ter", "ser", "foi", "fazer", "vamos", "tá", "né", "aí", "aqui", "então", "sobre", "villela",
            "reunião", "gente", "tudo", "bem", "sim", "vai", "falar", "porque", "agora", "coisa", "onde", "quem", "isso",
            "então", "aí", "pra", "pro", "né", "de", "vez", "seja", "pode", "dar", "ir", "dizer", "ver", "lá",
            "tipo", "vou", "era", "ia", "assim", "bom", "sei", "mim", "sabe", "acho", "cara", "sendo", 
            "vão", "temos", "tinha", "fiz", "fizemos", "faz", "fazemos", "dia", "ano", "meses", "aqui", "cara", "qual",
        ...
        if not HAS_CORPUS:
            ...
            for p in termos_positivos:
                c = full_text.count(p)
                if c > 0: contagem.append({"Termo": p.capitalize(), "Contagem": c, "Polaridade": "Positiva"})
            for n in termos_negativos:
                c = full_text.count(n)
                if c > 0: contagem.append({"Termo": n.capitalize(), "Contagem": c, "Polaridade": "Negativa"})
            ...
            if contagem:
                ...
                st.info("Nenhuma palavra da lista tática de negócios foi encontrada nessa amostra.")
        else:
            st.info("O volume de texto (corpus) extraído das reuniões é muito pequeno. Aumente a amostra no menu lateral.")
...
with tab6:
    ...
    st.markdown("Baseado na amostragem automatizada de reuniões transcritas pela Inteligência Artificial. Essa seção se molda ao **pior GAP atual** da equipe analisada.")
...
with tab7:
    ...
    st.markdown("Projete os resultados de faturamento e fechamento de contratos baseando-se no comportamento **real** da equipe lido através das estatísticas geradas por essa máquina.")
    ...
    with sc1:
        ...
        st.write(f"Considera a média real computada de conversão/avanço hoje da sua equipe: **{tx_atual*100:.1f}%**")
        st.metric(f"Fechamentos Mês", f"{fechamentos_atuais} contratos")
        st.metric(f"Receita Prevista Mês", f"R$ {faturamento_atual:,.2f}".replace(",", "_").replace(".", ",").replace("_", "."))
        
    with sc2:
        st.success("🚀 **Cenário Otimizado (Time forçando a Dor/Riscos e Implicação do SPIN)**")
        st.write(f"Considera a conversão média isolada dos consultores Villela que aplicam ganchos agressivos de **Implicação** nas calls atuais: (**{corr_implicacao_alta*100:.1f}%**)")
        st.metric(f"Fechamentos Mês", f"{fechamentos_otimizados} contratos", f"+{fechamentos_otimizados - fechamentos_atuais} contratos (Ajuste Tático)")
        aumento_rec = faturamento_otimizado - faturamento_atual
        st.metric(f"Receita Prevista Mês", f"R$ {faturamento_otimizado:,.2f}".replace(",", "_").replace(".", ",").replace("_", "."), f"+ R$ {aumento_rec:,.2f} recuperados da mesa".replace(",", "_").replace(".", ",").replace("_", "."))

    st.markdown("---")
    st.write("📌 *Como ler isso: O gap de receita entre o cenário base e o cenário otimizado é exatamente a quantidade de dinheiro que o grupo deixa na mesa por não blindar sua operação contra reuniões superficiais, longar e que não ferem a dor do cliente.*")
